...
class Matchdetail_mod extends CI_Model{
    ...
    public function get_players($mid){
		$lineup = (array)$this->get_lineup($mid);
        $lineup_players = array();
        $players = array();
        
        foreach($lineup as $k=>$v){
            if(preg_match("/^(hp|hs|ap|as)[1-9]/", $k)){
                $lineup_players[] = $v;
            }
        }
        
        $this->db->select('playerid, jersey, name');
        $this->db->where_in('playerid', $lineup_players);
        $query = $this->db->get('players');
        
        foreach($query->result() as $row){
            $players[$row->playerid] = $row->name . '(' . $row->jersey . ')';
        }
	...
	public function get_team_players($info){
        ...
        $action_name = isset($info['action_name']) ? $info['action_name']: '';
		$all_players = array();
		if(!empty($info['team_id']) && is_numeric($info['team_id'])){			
			...
			if($sc->num_rows() > 0){
				$scres = $sc->result();				
				if($scres[0]->hside == $info['team_id'] && $action_name != 'og'){
					$all_players['side'] = 'home';
				}
				else if($scres[0]->aside == $info['team_id'] && $action_name != 'og'){
					$all_players['side'] = 'away';
				}
                else if($scres[0]->aside == $info['team_id'] && $action_name == 'og'){
					$all_players['side'] = 'home';
				}
                else if($scres[0]->hside == $info['team_id'] && $action_name == 'og'){
					$all_players['side'] = 'away';
				}
				else{
    ...
    // update schedule table field 'status'
    public function update_schedule_status($mid){
        $data = array('status' => 2);
        
        $this->db->where(array('matchid' => $mid, 'status' => '0'));
        $this->db->or_where(array('status' => '1'));
        
        return $this->db->update('schedule', $data);
    } // end
    
    
    public function get_schedule($mid){
        $query = $this->db->get_where('schedule', array('matchid' => $mid));
        return $query->row();
    }
	...
	public function updatestatus($info){		
        ...
        $current_datetime = $date_time->format("Y-m-d H:i:s");
        $data = array();
        $data['status'] = $info['status'];
        
        if($data['status']==21){ // first half kickoff
            $schedule = $this->get_schedule($matchid);
            $exiting_utc_date_time = $schedule->utc;
            $utc_date_time = $exiting_utc_date_time;
            $datetime_field = 'utc';
        } elseif($data['status']==22 || $data['status']==32 || $data['status']==31){ // first half extra, second half kickoff & second half extra
            $ctime = $info['custom_time_sf'];
            // Identify field (utc or first_extra) for updating date time
            if($data['status'] == 22){
                $datetime_field = 'second_half';
            } elseif ($data['status'] == 32) {
                $datetime_field = 'second_extra';
            } elseif ($data['status'] == 31) {
                $datetime_field = 'first_extra';
            }
        }
        
        // Prepare date time
        if($ctime == '0') {
            $utc_date_time = $date_time->format("Y-m-d H:i:s");
        } elseif($ctime == '30') {
            $date_time->sub(new DateInterval('PT30S'));
            $utc_date_time = $date_time->format("Y-m-d H:i:s");
        } elseif($ctime == '1') {
            $date_time->sub(new DateInterval('PT1M'));
            $utc_date_time = $date_time->format("Y-m-d H:i:s");
        } elseif($ctime == '2') {
            $date_time->sub(new DateInterval('PT2M'));
            $utc_date_time = $date_time->format("Y-m-d H:i:s");
        } elseif($ctime == '3') {
            $date_time->sub(new DateInterval('PT3M'));
            $utc_date_time = $date_time->format("Y-m-d H:i:s");
        } elseif($ctime == '4') {
            $date_time->sub(new DateInterval('PT4M'));
            $utc_date_time = $date_time->format("Y-m-d H:i:s");
        } elseif($ctime == '5') {
            $date_time->sub(new DateInterval('PT5M'));
            $utc_date_time = $date_time->format("Y-m-d H:i:s");
        } elseif($ctime == 'custom') {
            $dt_custom = new DateTime($info['custom_time2']);
            echo $dt_custom->format("Y-m-d H:i:s") . '<br/>';
            $dt_custom->sub(new DateInterval('PT6H'));
            echo $dt_custom->format("Y-m-d H:i:s") . '<br/>';
            $utc_date_time = $dt_custom->format("Y-m-d H:i:s");
        }
        
//        echo '<pre>';
//        if($data['status']==21) echo 'exit utc time: ' . $exiting_utc_date_time . '<br />';
//        echo 'Current time: ' . $current_datetime . '<br />';
//        echo 'user inputed time: ' . $ctime . '<br />';
//        echo 'converted time for utc: ' . $utc_date_time . '<br />';
//        echo '</pre>';
        
        // date time filed in database
        if($data['status'] == 21 || $data['status'] == 22 || $data['status'] == 31 || $data['status'] == 32){
            $data[$datetime_field] = $utc_date_time;
        }
        
        $this->db->where('matchid', $matchid);
        $flag = $this->db->update('schedule', $data); 
    ...
    public function create_lineup($l_info){
		$info = array();	
		$cnt=1;
        ...
        foreach($info AS $k=>$v){
			$sql .= trim($k);
			$c++;
            $sql .= $c > count($info) &&  $c != count($info)? '' : ', ';
		}
        
        $sql .= ", stadium, hc, ac, referee";
        $sql .= ")  VALUES(";
        $sql .= "'" . $l_info['matchid'] . "', ";
	...
	public function update_lineup($l_info){
        ...
        $sql .= ", stadium ='" . $l_info['stadium'] . "'";
        $sql .= ", hc ='" . $l_info['hc'] . "'";
        $sql .= ", ac ='" . $l_info['ac'] . "'";
        $sql .= ", referee ='" . $l_info['referee'] . "' ";
	...
	public function get_team_players_by_team_id($team_id){
		...
		$sql = "SELECT `playerid`,`jersey`,`name`,`teamid`,`role` FROM `players` WHERE `teamid`='$team_id'";
		...
		if($f->num_rows() > 0){
			...
			foreach($res AS $v){
                ...
                // find coach
                if($v->role == 'C'){
                    $result['coach'] = $v->playerid;
                } else {
                    $result['players'][$v->playerid] = $v->name.' ('.$v->jersey.')';
                }
			}
	...
	public function update_match_action($info){
		...
		$sql = "UPDATE `match_details` SET `time`='".$info['time_munite']."',`action`='".$info['action']."',`player`='".$info['player']."',`assist_player`='".$info['assist_player']."',`actionfor`='".$info['action_for']."',`htscore`='".$info['halftime_score']."',`scorecard`='".$info['fulltime_score']."' WHERE `id`='".$info['mactionid']."'";		
	...
	public function add_match_action($info){
		...
		if($info['action']=='g' || $info['action']=='p' || $info['action']=='og'){
